...
module.exports = function SPCore(mod) {
	...
	mod.hook('C_PLAYER_LOCATION', 5, {order: 10, filter: {fake: null}}, event => {
		if(mod.settings.debug.loc) console.log('Location %d %d (%d %d %d %s) > (%d %d %d)', event.type, event.jumpDistance, Math.round(event.loc.x), Math.round(event.loc.y), Math.round(event.loc.z), degrees(event.w), Math.round(event.dest.x), Math.round(event.dest.y), Math.round(event.dest.z))
	...
	function startSkill(type, version, code, data) {
		...
		const event = protocol.parse(mod.protocolVersion, type, version, data = Buffer.from(data)),
	...
	function handleStartSkill(type, event, info, data) {
		...
		if(player.hasNocTan && !nocTanWarning) {
			console.log('[ESP] Noctenium Infusion detected in inventory.\n'
				+'      Warning: Having Noctenium Infusion in inventory can cause skill delays.\n'
				+'               This is a TERA bug and cannot be fixed by any module.\n'
				+'      Recommendation: Bank or discard all common Noctenium Infusions.\n'
				+'                      Uncommon, rare and superior versions do not trigger this bug.'
			)
			nocTanWarning = true
		}
		...
		if(info.abnormals)
			for(let id in info.abnormals)
				if(abnormality.exists(id)) {
					...
					if(abnormal.stamina) aStamina += abnormal.stamina
				}
	...
	mod.hook('S_ACTION_STAGE', 9, event => {
		if(isMe(event.gameId)) {
			if(mod.settings.debug.skills) {
				...
				if(mod.settings.debug.loc) strs.push(...[degrees(event.w), '(' + Math.round(event.loc.x), Math.round(event.loc.y), Math.round(event.loc.z) + ')'])
				...
				if(event.animSeq.length) {
					...
					for(let e of event.animSeq)
						movement.push(e.duration + ' ' + e.xyRate + ' ' + e.zRate + ' ' + e.distance)
			...
			if(info) {
				...
				if(info.forceClip && event.animSeq.length) {
					oopsPosition = applyDistance(lastStartPos, event.animSeq.map(m => m.distance).reduce((a, b) => a + b), (info.moveDir || 0) * Math.PI)
	...
	mod.hook('S_INSTANT_DASH', 3, event => {
		if(isMe(event.gameId)) {
			...
			if(serverAction && skillInfo(serverAction.skill)) {
				fixZ(event.loc)
				return false
			}
		}
		...
		if(isMe(event.gameId)) {
			...
			if(info) {
				if(info.dcType === 'dash')
					...
					else if(!lastEndPosition || lastEndPosition.loc.dist3D(event.loc) >= 80)
	...
	function sendActionStage(opts) {
		...
		let animSeq = opts.movement

		if(multiStage)
			animSeq = animSeq && animSeq[opts.stage] || !opts.moving && get(info, 'inPlace', 'movement', opts.stage) || get(info, 'movement', opts.stage) || []
		else
			animSeq = animSeq || !opts.moving && get(info, 'inPlace', 'movement') || info.movement || []
	...
	function skillInfo(skill) {
		...
		if(info[0] && info[0].dcType !== 'notimeline')
			console.log(`[SkillPrediction] Unemulated skill: ${player.templateId} ${skillId(skill)} (${info[0].dcType})`)
	...
	function fixZ(location) {
		if(location.x != 0 && location.y != 0 && location.z != 0)
			if(Math.abs(location.z - myPosition.loc.z) >= 40) myPosition.loc.z = location.z + 1
	}
}
